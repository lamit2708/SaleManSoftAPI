...
await new EfcSingleMigrateServiceSqlite<ProductFeatureDbContext, MProductFeatureEntity>().LogCustom(async (dbContext) =>
{
    var list = dbContext.Items.Where(x => x.Name == "Đặng Thế Nhân").Select(p => new MProductFeatureEntityBasic { Id = p.Id, Name = p.Name }).ToList();
    list.ForEach(data =>
    {
        if (data != null)
        {
