...
using VSoft.Company.ROL.Role.Client.Provider.Services;
using VSoft.Company.ROL.Role.Client.Services;
using Model = VSoft.Company.ROL.Role.Business.Dto.Data.RoleDto;
namespace VSoft.Company.ROL.Role.Api.UnitTest.Client.Bases
{
    public class TestMgmtClient : ApiClientTest<Model>
    {
        protected IRoleClient Client => GetService<IRoleClient>() ?? throw new Exception("Client is null!");       
        ...
        {
            ServiceCollection?.AddSingleton<MRoleClient>();
            ServiceCollection?.AddSingleton(new TokenService().Init(Token));
            ServiceCollection?.AddSingleton<IRoleClient, RoleClient>();
        }
        ...
        protected async Task TestFindAsync(MDtoRequestFindByString request)
        {
            await RunTest("TestFindAsync", async (log) =>
            {
                ...
                var client = GetService<IRoleClient>();            
