...
using Model = VSoft.Company.PRC.ProductCategory.Business.Dto.Data.ProductCategoryDto;
namespace VSoft.Company.PRC.ProductCategory.Api.UnitTest.Client.Bases
{
    public class TestMgmtClient : ApiClientTest<Model>
    {
        protected IProductCategoryClient Client => GetService<IProductCategoryClient>() ?? throw new Exception("Client is null!");
        ...
        {
            ServiceCollection?.AddSingleton<MProductCategoryClient>();
            ServiceCollection?.AddSingleton(new TokenService().Init(Token));
            ServiceCollection?.AddSingleton<IProductCategoryClient, ProductCategoryClient>();
        }
        ...
        protected async Task TestFindAsync(MDtoRequestFindByString request)
        {
            await RunTest("TestFindAsync", async (log) =>
            {
                ...
                var client = GetService<IProductCategoryClient>();
        ...
        protected async Task TestCreateAsync(ProductCategoryInsertDtoRequest request)
        {
        ...
        protected async Task TestCreateRangeAsync(ProductCategoryInsertRangeDtoRequest request)
        {
        ...
        protected async Task TestUpdateAsync(ProductCategoryUpdateDtoRequest request)
        {
        ...
        protected async Task TestUpdateRangeAsync(ProductCategoryUpdateRangeDtoRequest request)
        {
        ...
        protected async Task TestDeleteAsync(ProductCategoryDeleteDtoRequest request)
        {
        ...
        protected async Task TestDeleteRangeAsync(ProductCategoryDeleteRangeDtoRequest request)
        {
        ...
        protected async Task TestSaveRangeAsync(ProductCategorySaveRangeDtoRequest request)
        {
