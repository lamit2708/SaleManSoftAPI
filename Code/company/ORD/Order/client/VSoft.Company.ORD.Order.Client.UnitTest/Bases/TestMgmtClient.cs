...
using VSoft.Company.ORD.Order.Client.Provider.Services;
using VSoft.Company.ORD.Order.Client.Services;
using Model = VSoft.Company.ORD.Order.Business.Dto.Data.OrderDto;
namespace VSoft.Company.ORD.Order.Api.UnitTest.Client.Bases
{
    public class TestMgmtClient : ApiClientTest<Model>
    {
        protected IOrderClient Client => GetService<IOrderClient>() ?? throw new Exception("Client is null!");       
        ...
        {
            ServiceCollection?.AddSingleton<MOrderClient>();
            ServiceCollection?.AddSingleton(new TokenService().Init(Token));
            ServiceCollection?.AddSingleton<IOrderClient, OrderClient>();
        }
        ...
        protected async Task TestFindAsync(MDtoRequestFindByString request)
        {
            await RunTest("TestFindAsync", async (log) =>
            {
                ...
                var client = GetService<IOrderClient>();            
