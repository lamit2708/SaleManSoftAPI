...
using Model = VSoft.Company.DQU.DealQuotation.Business.Dto.Data.DealQuotationDto;
namespace VSoft.Company.DQU.DealQuotation.Api.UnitTest.Client.Bases
{
    public class TestMgmtClient : ApiClientTest<Model>
    {
        protected IDealQuotationClient Client => GetService<IDealQuotationClient>() ?? throw new Exception("Client is null!");       
        ...
        {
            ServiceCollection?.AddSingleton<MDealQuotationClient>();
            ServiceCollection?.AddSingleton(new TokenService().Init(Token));
            ServiceCollection?.AddSingleton<IDealQuotationClient, DealQuotationClient>();
        }
        ...
        protected async Task TestFindAsync(MDtoRequestFindByString request)
        {
            await RunTest("TestFindAsync", async (log) =>
            {
                ...
                var client = GetService<IDealQuotationClient>();            
        ...
        protected async Task TestCreateAsync(DealQuotationInsertDtoRequest request)
        {
        ...
        protected async Task TestCreateRangeAsync(DealQuotationInsertRangeDtoRequest request)
        {
        ...
        protected async Task TestUpdateAsync(DealQuotationUpdateDtoRequest request)
        {
        ...
        protected async Task TestUpdateRangeAsync(DealQuotationUpdateRangeDtoRequest request)
        {
        ...
        protected async Task TestDeleteAsync(DealQuotationDeleteDtoRequest request)
        {
        ...
        protected async Task TestDeleteRangeAsync(DealQuotationDeleteRangeDtoRequest request)
        {
        ...
        protected async Task TestSaveRangeAsync(DealQuotationSaveRangeDtoRequest request)
        {
