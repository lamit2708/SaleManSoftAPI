...
public class TestProductReal : TestMgmtEntities
{
    ...
    public async Task CreateSeedDataAsync(int id, string name, double price, int quantity, int categoryId, string description)
    {
    ...
    [TestMethod]
    [DataRow(1, "Hệ thống quản lý nhân viên, chấm công 123", 200000000, 1, 1, "Hệ thống giúp thu thập, giải quyết, lưu trữ, truyền đạt, phân phối các thông tin có liên quan đến nguồn nhân lực trong công ty để hỗ trợ cho việc ra quyết định." )]
    [DataRow(2, "Hệ thống chăm sóc khách hàng 321", 100000000, 1, 1, "Hệ thống CRM như một kho lưu trữ duy nhất để kết hợp các hoạt động bán hàng, tiếp thị, hỗ trợ khách hàng, giúp hợp lý hóa quy trình, chính sách và nhân lực trong một nền tảng." )]
    [DataRow(3, "Hệ thống quản lý phòng trọ", 20000000, 1, 1, "Mọi thông tin về khách thuê trọ chi tiết có thể lưu trữ khi sử dụng phần mềm quản lý nhà trọ. Từ thông tin cá nhân tới hợp đồng cho thuê (giá cả, phí dịch vụ, thời gian, tiền cọc, số người ở…)." )]
    [DataRow(4, "Hệ thống quản lý trung tâm ngoại ngữ", 20000000, 1, 1, "Phần mềm quản lý giúp ban lãnh đạo có thể nắm bắt được tình hình trung tâm, tình trạng học viên, đưa ra cái nhìn tổng quan cho chất lượng giảng dạy của các giảng viên." )]
    [DataRow(5, "Hệ thống quản lý quán cà phê", 20000000, 1, 1, "Phần mềm quản lý quán cafe giúp tính tiền, quản lý kho, quản lý dữ liệu khách hàng, quản lý nhân viên." )]
    [DataRow(6, "Hệ thống quản lý nhà hàng", 100000000, 1, 1, "Phần mềm quản lý bàn, hỗ trợ in bếp, quản lý nhân viên, quản lý kho" )]
    ...
    public async Task UpdateAsync(int id, string name, double price, int quantity, int categoryId, string description)
    {
        ...
        e.CategoryId = categoryId;
        e.Description = description;
        await RunTest("UpdateWithKeywordAsync", async (r, l) =>
        {
            var res = r?.UpdateWithKeyword(e);
            LogEntity(res, l);
        });
    }
